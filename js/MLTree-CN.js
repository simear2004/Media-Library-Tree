...
function getVisibleNodes() {
	if (search.text && search.visibleIds) {
		...
		function traverse(list, level) {
			for (const node of list) {
				if (search.visibleIds.has(node.id)) {
					node.level = level;
					result.push(node);
		...
		if (tree.data.length) traverse([tree.data[0]], 0);
		return result;
	}
	if (tree.cachedVisibleDirty || !tree.cachedVisible) {
		...
		function traverse(list, level) {
			for (const node of list) {
				node.level = level;
				tree.cachedVisible.push(node);
				if (node.expanded && node.children.length) traverse(node.children, level + 1);
			}
		}
		if (tree.data.length) traverse([tree.data[0]], 0);
	...
	function markVisible(node) {
		...
		if (node.type === 'track') {
			...
			for (let i = 0; i < node.children.length; i++) {
				matchCount += markVisible(node.children[i]);
			}
		}
...
function expandAll() {
	...
	if (tree.data.length > 0) expandNode(tree.data[0]);
	invalidateVisibleNodesCache();
	window.Repaint();
}
